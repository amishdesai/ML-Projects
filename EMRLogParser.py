...
- Optionally enriches top clusters with an LLM (OpenAI Responses API)
...
def s3_list_keys(bucket: str, prefix: str, days: int = 2, max_keys: int = 10000) -> List[str]:
    """
...
def s3_read_text(bucket: str, key: str, byte_limit: int = 2_000_000) -> str:
    """
...
def llm_analyze_cluster(
    ...
    """
    Calls OpenAI to summarize root cause + fixes for one cluster.
    Expects the model to return JSON. Falls back to raw text if parsing fails.
    """
    logging.info(f"Analyzing cluster {cluster.cluster_id} ({cluster.exception_class}) with LLM...")
    payload = {
    ...
    resp = client.responses.create(
        # Use a model you have access to. If you prefer, change to "gpt-4.1" or similar.
    ...
    try:
        ...
        logging.info(f"LLM analysis completed for cluster {cluster.cluster_id}")
        return result
    except Exception:
        logging.warning(f"Failed to parse LLM output as JSON for cluster {cluster.cluster_id}")
...
def run(
    ...
    for label, pfx in prefixes:
        ...
        probe = s3_list_keys(bucket, pfx, days=days, max_keys=1)
        if not probe:
            logging.info(f"No files found in {pfx}")
            files_per_prefix[pfx] = 0
            continue

        keys = s3_list_keys(bucket, pfx, days=days, max_keys=10000)
        ...
        for idx, key in enumerate(keys, 1):
            ...
            txt = s3_read_text(bucket, key, byte_limit=read_byte_limit)
    ...
    if use_llm:
        ...
        for idx, c in enumerate(clusters[:max_llm], 1):
            try:
                ...
                analysis = llm_analyze_cluster(client, c, spark_version)
...
def main():
    ...
    if len(argv) < 3:
        print(
            "Usage: python EMRLogParser.py <S3_BUCKET> <CLUSTER_PREFIX e.g. j-XXXX/> [spark_version]",
            file=sys.stderr,
        )
        print(
            "Optional env: EMR_DAYS=2 EMR_USE_LLM=1 EMR_READ_BYTES=2000000 EMR_MAX_BLOCKS_PER_FILE=200 EMR_MAX_LLM=10 EMR_OUT=-|path.json",
            file=sys.stderr,
        )
        raise SystemExit(2)
    ...
    spark_version = argv[3] if len(argv) > 3 else None

    out_path = (os.environ.get("EMR_OUT") or "-").strip() or "-"
    ...
    if use_llm and not os.environ.get("OPENAI_API_KEY"):
        logging.warning("EMR_USE_LLM=1 but OPENAI_API_KEY not set, disabling LLM")
        use_llm = False

    logging.info(f"Configuration: days={days}, read_bytes={read_bytes}, max_blocks_per_file={max_blocks_per_file}")
